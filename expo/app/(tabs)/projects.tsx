...
import { MapPin, Users, ShieldCheck, Search, Filter, Plus, Star } from "lucide-react-native";
...
function UserProjectCard({ project }: { project: UserProject }) {
  const prog = project.goal > 0 ? project.raised / project.goal : 0;
  const cc = categoryColors[project.category] ?? Colors.dark.accent;
  return (
    <TouchableOpacity style={st.card} activeOpacity={0.8} onPress={() => router.push("/manage-funds")} testID={`user-project-${project.id}`}>
      <Image source={{ uri: project.imageUrl }} style={st.cardImage} contentFit="cover" />
      <View style={st.cardBody}>
        <View style={st.cardTopRow}>
          <View style={[st.categoryTag, { backgroundColor: cc + "20" }]}><View style={[st.catDot, { backgroundColor: cc }]} /><Text style={[st.categoryText, { color: cc }]}>{project.category.charAt(0).toUpperCase() + project.category.slice(1)}</Text></View>
          <View style={st.userBadge}><Star size={10} color={Colors.dark.warning} /><Text style={st.userBadgeText}>{project.published ? "Live" : "Draft"}</Text></View>
        </View>
        <Text style={st.cardTitle} numberOfLines={2}>{project.name}</Text>
        <View style={st.cardMeta}><View style={st.metaItem}><MapPin size={12} color={Colors.dark.textMuted} /><Text style={st.metaText}>{project.location}</Text></View></View>
        <View style={st.progressSection}>
        ...
        {project.earnings > 0 && (
          <View style={st.earningsRow}><Text style={st.earningsLabel}>Available:</Text><Text style={st.earningsValue}>${project.earnings.toLocaleString()}</Text></View>
        )}
      </View>
    </TouchableOpacity>
  );
...
export default function ImpactChainProjects() {
  ...
  const allProjects = useMemo(() => {
    const userAsProjects: Project[] = userProjects.map((p) => ({
      ...p,
    }));
    return [...userAsProjects, ...projects];
  }, [userProjects]);
  ...
  const filtered = useMemo(() => allProjects.filter((p) => {
    ...
    return ms && mc;
  }), [search, cat, allProjects]);

  const isUserProject = useCallback((p: Project) => userProjects.some((up) => up.id === p.id), [userProjects]);

  const renderItem = useCallback(({ item }: { item: Project }) => {
    const up = userProjects.find((u) => u.id === item.id);
    if (up) return <UserProjectCard project={up} />;
  ...
  return (
    <View style={st.container}>
      <View style={st.searchRow}>
        <View style={st.searchBox}><Search size={16} color={Colors.dark.textMuted} /><TextInput style={st.searchInput} placeholder="Search projects..." placeholderTextColor={Colors.dark.textMuted} value={search} onChangeText={setSearch} /></View>
        <TouchableOpacity style={st.createBtn} activeOpacity={0.8} onPress={() => router.push("/create-project")} testID="create-project-btn">
...
const st = StyleSheet.create({
  ...
  searchRow: { flexDirection: "row", paddingHorizontal: 20, paddingTop: 8, paddingBottom: 8, gap: 10 },
  searchBox: { flex: 1, flexDirection: "row", alignItems: "center", backgroundColor: Colors.dark.surface, borderRadius: 12, paddingHorizontal: 14, height: 44, borderWidth: 1, borderColor: Colors.dark.border, gap: 10 },
  createBtn: { width: 44, height: 44, borderRadius: 12, backgroundColor: Colors.dark.accent, alignItems: "center", justifyContent: "center" },
  userBadge: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8, backgroundColor: Colors.dark.warningMuted },
  userBadgeText: { fontSize: 11, fontWeight: "600" as const, color: Colors.dark.warning },
  earningsRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: Colors.dark.border },
  earningsLabel: { fontSize: 12, color: Colors.dark.textMuted },
  earningsValue: { fontSize: 14, fontWeight: "700" as const, color: Colors.dark.accent },
